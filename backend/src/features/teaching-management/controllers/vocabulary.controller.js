...
const getVocabularies = async (req, res) => {
  try {
    const {
      ...
      vocabulary_type,
      is_private,
      status, // Match service's status field
    } = req.query;
...
const getVocabulariesByTopic = async (req, res) => {
  try {
    ...
    const result = await vocabularyService.getVocabulariesByTopicId(
      parseInt(topic_id),
    );
...
const getVocabulariesByType = async (req, res) => {
  try {
    ...
    const result = await vocabularyService.getVocabulariesByType(
      vocabulary_type,
      {
        topic_id: topic_id ? parseInt(topic_id) : null,
        classroom_id: classroom_id ? parseInt(classroom_id) : null,
...
const getVocabularyById = async (req, res) => {
  try {
    ...
    const result = await vocabularyService.getVocabularyById(
